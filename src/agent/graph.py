...
def stream_final_answer(messages: list[BaseMessage]) -> Iterator[str]:
    """
    Streaming output final answer。

    Args:
        messages: Message list (contains historical and current conversations）

    Yields:
        answer to each token
    """
    prompt_messages = [
...
def run_agent_loop_events(
    ...
    """
    Execute Agent loop and generate structured events。
    ...
    """
    # Build message list: historical messages + Current issues
...
def stream_answer_events(
    ...
    """
    Execute the Agent and stream events back。

    Args:
        question: User issues
        history: Historical message list
