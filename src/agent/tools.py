...
def _build_qdrant_filter(
    ...
    Args:
        pdf_name: Paper name filter
        chunk_types: chunk Type list filter
        page_idx: Exact page number filter
        page_start/page_end: Page range filter
        filter_metadata: JSON Additional metadata filters for formats

    Returns:
        models.Filter Object, returned if unconditional None
    """
    import json

    must_conditions: list[models.Condition] = []

    # Parse filter_metadata JSON and add conditions
...
def _payload_to_evidence(
    ...
    """Convert from Qdrant payload to evidence format。"""
