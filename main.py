...
def add_paper(pdf_path: str) -> None:
    ...
    multimodal_inputs, metadata_list, parsed_data = process_paper(pdf_path, save_markdown=True)
    get_vector_store().add_multimodal(multimodal_inputs, metadata_list)
    
    pdf_name = parsed_data.get("pdf_name", "")
    if pdf_name:
        from api.services.paper_service import _persist_pdf_for_reader
        _persist_pdf_for_reader(pdf_path, pdf_name)
